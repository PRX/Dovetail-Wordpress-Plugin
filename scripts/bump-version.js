...
function getCurrentVersion() {
  ...
  if (!fs.existsSync(constantsPath)) {
    console.log(
      chalk.yellow(
        "Warning: constants.php not found, falling back to package.json",
      ),
    );
  ...
  // Try to find the DTPODCASTS_VERSION constant
  const dtPodcastsMatch = contents.match(
    /define\(\s*'DTPODCASTS_VERSION',\s*'([^']+)'\s*\)/,
  );

  if (dtPodcastsMatch) {
    return dtPodcastsMatch[1];
  }

  // If no version found in constants.php, try package.json
  console.log(
    chalk.yellow(
      "Warning: No version constant found in constants.php, falling back to package.json",
    ),
  );
  const packageJson = require(path.join(process.cwd(), "package.json"));
  return packageJson.version;
...
function determineBumpType() {
  try {
    ...
    const bumpType = execSync("node scripts/analyze-changesets.js", {
      encoding: "utf8",
    }).trim();
...
function updateConstantsFile(newVersion) {
  ...
  if (!fs.existsSync(filePath)) {
    console.log(
      chalk.yellow("Warning: constants.php not found, skipping update"),
    );
    return;
  }
  ...
  // Update DTPODCASTS_VERSION with WordPress coding standards formatting
  if (content.includes("DTPODCASTS_VERSION")) {
    content = content.replace(
      /define\(\s*'DTPODCASTS_VERSION',\s*'[^']+'\s*\)/,
      `define( 'DTPODCASTS_VERSION', '${newVersion}' )`,
    );
    ...
    console.log(
      chalk.yellow(
        "Warning: DTPODCASTS_VERSION constant not found in constants.php",
      ),
    );
  }
}
...
/**
 * Update version in dovetail-podcasts.php
 *
 * @param {string} newVersion New version
 */
function updatePluginFile(newVersion) {
  const filePath = path.join(process.cwd(), "dovetail-podcasts.php");
  let content = fs.readFileSync(filePath, "utf8");
  ...
  fs.writeFileSync(filePath, content);
  console.log(`Updated version in dovetail-podcasts.php to ${newVersion}`);
}
...
function updateReadmeTxt(newVersion) {
  ...
  if (fs.existsSync(filePath)) {
    ...
    content = content.replace(/Stable tag: .+/, `Stable tag: ${newVersion}`);
