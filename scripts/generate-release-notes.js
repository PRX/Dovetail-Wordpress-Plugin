...
const argv = yargs(hideBin(process.argv))
  ...
  .option("token", {
    type: "string",
    description:
      "GitHub token for API requests (optional, helps avoid rate limits)",
    default: "",
  })
  .help().argv;
...
function getRepositoryUrl() {
  ...
  try {
    ...
    if (packageJson.name && packageJson.name.includes("/")) {
      ...
      return `https://github.com/PRX/${packageJson.name}`;
    }

    console.warn("Could not determine repository URL from package.json");
    return "https://github.com/PRX/Dovetail-Wordpress-Plugin";
  } catch (err) {
    console.warn(
      "Could not determine repository URL from package.json:",
      err.message,
    );
    return "https://github.com/PRX/Dovetail-Wordpress-Plugin";
  }
...
async function isFirstTimeContributor(username, repoInfo) {
  try {
    ...
    if (!searchResult) {
      console.warn(
        `Could not determine if ${username} is a first-time contributor.`,
      );
      return false;
    }
    ...
    console.warn(
      `Error checking if ${username} is a first-time contributor:`,
      err,
    );
    return false;
  }
...
async function getContributorInfo(changesets) {
  ...
  const authors = [
    ...new Set(changesets.map((changeset) => changeset.author)),
  ].filter(Boolean);
  ...
  if (token) {
    ...
    console.warn(
      "No GitHub token provided. Skipping first-time contributor check.",
    );
  }
...
function categorizeChangesets(changesets) {
  ...
  const breakingChanges = changesets.filter((changeset) =>
    isBreakingChange(changeset),
  );

  const features = changesets.filter(
    (changeset) => changeset.type === "feat" && !isBreakingChange(changeset),
  );

  const fixes = changesets.filter(
    (changeset) => changeset.type === "fix" && !isBreakingChange(changeset),
  );

  const other = changesets.filter(
    (changeset) =>
      !isBreakingChange(changeset) &&
      changeset.type !== "feat" &&
      changeset.type !== "fix",
